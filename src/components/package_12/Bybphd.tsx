...
export default class Bybphd extends React.Component<Props,State>{
    state = {
        info:{
            flow_rate:0,
            start:0,
        ...
        return (
            <div className ="Bybphd" >
                ...
                <div className = "rule">
                    ...
                    <p>3. 参加活动的玩家只能进行《疯狂旋涡》《捕鱼·海王》《捕鱼·聚宝盆》指定游戏，进行其他游戏视为放弃活动。</p>
    ...
    getLocalApply(){
        let local = localStorage.getItem(`ApplyBybphd_${gHandler.UrlData.user_id}`)
        if(local){
            return false
        }else{
            return true
    ...
    setLocalApply(){
        localStorage.setItem(`ApplyBybphd_${gHandler.UrlData.user_id}`,`${true}`)            
    }
    setLocal(){
