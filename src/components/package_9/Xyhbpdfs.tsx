...
export default class Xyhbpdfs extends React.Component<Props,State>{
    state = {
        info:{
            flow_rate:0,
            start:0,
    ...
    getLocalApply(){
        let local = localStorage.getItem(`ApplyXyhbpdfs_${gHandler.UrlData.user_id}`)
        if(local){
            return false
        }else{
            return true
    ...
    setLocalApply(){
        localStorage.setItem(`ApplyXyhbpdfs_${gHandler.UrlData.user_id}`,`${true}`)            
    }
    setLocal(){
