...
export default class Lyhsc35 extends React.Component<Props,State>{
    state = {
        info:{
            range:[
                {
    ...
    componentDidMount(){
        ...
        this.Axios_reqgetSecondPayBonusGold()
    }
    renderBtn(){
    ...
    onClick =(e:any)=>{
        this.Axios_reqreceiveSecondPayBonusGold()
    }
    private async Axios_reqreceiveSecondPayBonusGold(){
        let url = `${gHandler.UrlData.host}${Api.reqreceiveSecondPayBonusGold}`;
    ...
    private async  Axios_reqgetSecondPayBonusGold(){
        let url = `${gHandler.UrlData.host}${Api.reqgetSecondPayBonusGold}`;
        let data = new FormData();
        data.append('user_id',gHandler.UrlData.user_id);
        data.append('activity_id',this.props.curData.id);
        data.append('center_auth',gHandler.UrlData.center_auth);
    ...
    render (){
        ...
        return (
            <div className ={`${!gHandler.getDeviceisIphone() && gHandler.UrlData.client == "h5"?"Lyhsc35h5":"Lyhsc35"}`} >
                ...
                <div className ="liushui">
                    <div style={{display:"flex",justifyContent:'center'}}>{`本金1倍+`}</div>
                    <div>{`彩金${this.state.info.flow_rate}倍流水`}</div>
                </div>
                <div className = "rule">
                    {/* <p>1.本活动需要完成手机和银行卡绑定后才能参与。</p>
                    <p>2.游戏规则：仅参加以下游戏《财神到》《水果机》《捕鱼·海王》《捕鱼·聚宝盆》《多福多财》《疯狂旋涡》《CQ9电子游戏》《AG电子游戏》《PT电子游戏》《JDB电子游戏》《PG电子游戏》《PG2电子游戏》《PP电子游戏》《MG电子游戏》。</p>
