...
export default class FfcBaoPeiQiQu extends React.Component<Props,State>{
    state = {
        info:{
            flow_rate:0,
            start:0,
    ...
    getLocalApply(){
        let local = localStorage.getItem(`ApplyFfcBaoPeiQiQu_${gHandler.UrlData.user_id}`)
        if(local){
            return false
        }else{
            return true
    ...
    setLocalApply(){
        localStorage.setItem(`ApplyFfcBaoPeiQiQu_${gHandler.UrlData.user_id}`,`${true}`)            
    }
    setLocal(){
