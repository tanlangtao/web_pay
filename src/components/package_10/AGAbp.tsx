...
export default class AGAbp extends React.Component<Props,State>{
    state = {
        info:{
            flow_rate:0,
            start:0,
        ...
        return (
            <div className ="AGAbp" style={{
                ...
                <div className = "rule">
                    ...
                    <p>3. 参加活动的玩家只能AGA电子游戏<span style={{color:"#F3DC5B"}}>《多福多财》《城堡争霸》《疯狂旋涡》《云谷寻宝》</span>，进行其他游戏视为放弃活动。</p>
    ...
    getLocalApply(){
        let local = localStorage.getItem(`ApplyPayAGABaoPei_${gHandler.UrlData.user_id}`)
        if(local){
            return false
        }else{
            return true
    ...
    setLocalApply(){
        localStorage.setItem(`ApplyPayAGABaoPei_${gHandler.UrlData.user_id}`,`${true}`)            
    }
    setLocal(){
