...
import BtnLightBox29 from './BtnLightBox';
import Jtfl29 from 'components/package_29/Jtfl';
import DailySign29 from 'components/package_29/DailySign';
import Lyhsc29 from 'components/package_29/Lyhsc';
import Xyhschd29 from 'components/package_29/Xyhschd';
import Xyhbp29 from 'components/package_29/Xyhbp';
interface State{
    loading:Boolean,
    navArr :ConfigItem[],
    title :string,
...
export default class Activity29 extends Component<{}, State> {
    state = {
        navArr:[],
        title:'',
        curData:{
    ...
    render() {
            ...
            return this.state.navArr.map((item:ConfigItem,index)=>{
                ...
                >
                    <div  onClick={()=>{
                        ...
                        {
                            item.name ===this.state.title?<BtnLightBox29></BtnLightBox29>:null
                        }
                    </div>
                </SwiperSlide>
            })
        ...
        if(this.state.curData.id ===""){
            return <div className='Activity29'>
        ...
        return (
            !this.state.loading ?<div className='Activity29'>
                <div className ="contentBox">
                    ...
                    <div className="content" style={{
                        ...
                        {
                            (this.state.title==='集团福利29' ? <Jtfl29 curData={this.state.curData}/>:
                                (this.state.title==='每日免费礼金29' ? <DailySign29 curData={this.state.curData}/>:
                                    (this.state.title==='老用户首存活动29' ? <Lyhsc29 curData={this.state.curData}/>:
                                        (this.state.title==='新用户首存活动29' ? <Xyhschd29 curData={this.state.curData}/>:
                                            (this.state.title==='新用户包赔活动29' ? <Xyhbp29 curData={this.state.curData}/>:
                                                <div> </div>
                                            )
                                        )
                                    )
